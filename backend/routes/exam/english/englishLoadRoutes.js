...
router.get("/:examId/load", auth, async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Load ordered questions (passage + objective)
    ...
    // 3️⃣ Group questions by section
    const sections = [];
    ...
    qRes.rows.forEach((row) => {
      ...
      if (!section) {
        section = {
          ...
          title:
            row.passage_title ||
            (row.question_type === "objective" ? "OBJECTIVE" : "PASSAGE SECTION"),
          passage: row.passage || "",
          questions: [],
        };
    ...
    return res.status(500).json({ message: "Failed to load English exam" });
  }
});
